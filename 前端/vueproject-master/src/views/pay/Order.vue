    ...
    <el-table size="small" :data="listData" highlight-current-row v-loading="loading" border element-loading-text="拼命加载中" style="width: 100%;">
      ...
      <el-table-column align="center" label="操作" min-width="150">
        <template slot-scope="scope">
          <el-button v-if="scope.row.bStatus==='已支付'" size="mini" type="primary" @click="deliverOrderById(scope.$index, scope.row)">发货</el-button>
          <el-button v-if="scope.row.bStatus==='已取消'||scope.row.bStatus==='已签收'" size="mini" type="danger" @click="deleteOrder(scope.$index, scope.row)">删除</el-button>        
        </template>
      </el-table-column>
...
<script>
import { OrderList, OrderSave, OrderDelete, GetOrderById,deliverById} from '@/api/payMG.js'
      ...
      this.$confirm('确定要发货吗?', '信息', {
        ...
        .then(() => {
          deliverById(row.bId)
            ...
            .catch(err => {
              this.loading = false
              this.$message.error('订单发货失败，请稍后再试！')
            })
        })
        .catch(() => {
          this.$message({
